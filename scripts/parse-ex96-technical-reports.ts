#!/usr/bin/env npx tsx
/**
 * Parse EX-96.1 technical reports from edgar_technical_documents
...
async function processDocument(doc: any): Promise<void> {
  ...
  const finalData: ExtractedData = {
    ...
    primary_commodity: aiData.primary_commodity?.toLowerCase(),
...
async function main() {
  console.log('🚀 STARTING EX-96.1 TECHNICAL REPORT PARSER');
  ...
  try {
    ...
    const { data: documents, error } = await supabase
      ...
      .limit(100); // Process first 100 for testing
